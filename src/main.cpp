...
void loop1() {
	...
	if (pidCycleTimer >= 1000000 / PID_RATE) {
		...
		if (gyroCycle) {
			...
			// Update ML predictor with new IMU data (every gyro cycle = 1600 Hz)
#ifdef ENABLE_ML_LOGGER
			MLPredictor::logSample(accelDataRaw[0], accelDataRaw[1], accelDataRaw[2],
								   gyroDataRaw[0], gyroDataRaw[1], gyroDataRaw[2],
								   triggerState);
#else
			MLPredictor::updateIMU(accelDataRaw[0], accelDataRaw[1], accelDataRaw[2],
								   gyroDataRaw[0], gyroDataRaw[1], gyroDataRaw[2]);
			MLPredictor::predict();
			MLPredictor::updateTimeout();
#endif
