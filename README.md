...
Recording is controlled from **Menu → Device**:
- `ML Recording: ON/OFF` (status)
- `Start ML Recording`
- `Stop ML Recording`

The log is a single “session”. It is cleared on reboot/firmware flash, so dump it before power-cycling.
...
| `python/ml_model_export.py` | Trains + exports on-device model files (`MLMD` format) for serial upload (LR + MLP) |
| `python/ml_model_push.py` | Uploads and loads user models over serial (defaults to uploading **both** LR+MLP) |
...
Factory weights are stored in `src/mlWeights.h`. The firmware also supports loading a user-provided model from LittleFS (`/ml_model.bin`) at runtime (no UF2 reflashing).

### Uploading a personalized model (no UF2)

The firmware exposes serial commands:
- `MLMODEL_PUT_LR <size> <crc32hex>` / `MLMODEL_PUT_MLP <size> <crc32hex>` (device replies `MLMODEL_READY`, then you stream `<size>` raw bytes; device replies `MLMODEL_OK`)
- `MLMODEL_LOAD_LR` / `MLMODEL_LOAD_MLP` (loads `/ml_model_lr.bin` / `/ml_model_mlp.bin` into RAM; replies `MLMODEL_LOADED`)
- `MLMODEL_INFO`, `MLMODEL_DELETE`

Legacy single-file mode is also supported (`MLMODEL_PUT`, `MLMODEL_LOAD` writing `/ml_model.bin`), but new tooling defaults to uploading both models.

To export and upload a model from your local training dataset:
...
### “Default experience” (single-session personalization)

This is the intended end-user workflow (lowest friction, one session type):
1. **Record once**: start ML Recording, play normally for a few minutes, then stop ML Recording. Include some **negative time** (aiming/moving without shooting) during the same session.
2. **Plug in + pull log**: dump the session with `python/python/ml_log_pull.py` and convert to CSV.
3. **Inspect**: run `python/python/ml_log_explore.py` to sanity-check trigger timing and motion.
4. **Train both**: export both `ml_model_lr.bin` + `ml_model_mlp.bin` with `python/python/ml_model_export.py --model both`.
5. **Upload without reflashing**: `python/python/ml_model_push.py` uploads + loads both models; verify with `python/python/ml_model_push.py --info`.

If you later build a website UI, it should follow the same sequence: pull log → visualize → train → upload weights → verify, without requiring the user to unplug/replug or flash a UF2.
